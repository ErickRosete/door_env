...
from door.env.sawyer_gripper import SawyerGripper
from door.env.cameras.static_camera import StaticCamera
from door.utils.path import pkg_path
...
class SawyerDoorEnv(gym.Env):
    ...
    def load_objects(self):
        ...
        if "env" not in self.cfg.sawyer_gripper.robot_params.urdf_path:
            robot_urdf_path = pkg_path("../" + self.cfg.sawyer_gripper.robot_params.urdf_path)
...
def register_sawyer_door_env(max_episode_steps=1000):
    gym.envs.register(
        id='sawyer-door-v0',
        entry_point='door.env.sawyer_door_env:SawyerDoorEnv',
        max_episode_steps=max_episode_steps,
    )

def custom_sawyer_grasp_env(cfg):
    env = gym.make('sawyer-door-v0', cfg=cfg)
...
def env_test(cfg):
    env = custom_sawyer_grasp_env(cfg.env)
